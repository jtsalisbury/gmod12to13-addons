...
function PooPee.DoPoo(ply)
    if not ply:Alive() or ply.DarkRPVars.Poop < 30 then
        DarkRP.notify(ply,1,6, string.format(LANGUAGE.unable, "/poo", ""))
        return ""
    end
...
DarkRP.defineChatCommand("poo", PooPee.DoPoo)
DarkRP.defineChatCommand("poop", PooPee.DoPoo)

function PooPee.DoPee(ply)
    if GetGlobalInt("poopeemod") ~= 1 then
        DarkRP.notify(ply,1,4, string.format(LANGUAGE.disabled, "/pee", ""))
        return ""
    end
...
DarkRP.defineChatCommand("pee", PooPee.DoPee)
